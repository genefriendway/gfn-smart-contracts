...
async function main() {
    ...
    await approveRouter(usdtToken, account.address, routerPancakeOptions.address, approveAmount)

    // Swap USDT -> PCSP
    for (let i = 0; i < 3; i++) {
        await swapToken(routerPancake, account.address, account.address, amountIn, usdtTokenOptions.address, pcspTokenOptions.address)
    }
...
async function approveRouter(tokenContract, fromAccount, router, amount) {
    const approve = await tokenContract.methods.approve(
        router,
        amount
    ).send({ from: fromAccount })

    console.log('Approval: ', approve.transactionHash)
}

async function swapToken(routerContract, fromAccount, toAccount, amount, fromToken, toToken) {
    const amountOutMin = utils.toWei('0', 'ether') // calculate based on price
    const path = [fromToken, toToken]
    const currentTimestamp = await getCurrentTimestamp()
    const deadline = currentTimestamp + 60 * 10;    // 10mins from current time

    const swap = await routerContract.methods.swapExactTokensForTokens(
        amount,
        amountOutMin,
        path,
        toAccount,
